...
- Stage12 packages the fixed Stage10/11 results into [a stakeholder-ready report](reports/spy_risk_alert_stakeholder_report.md). It states the recommended operating boundary, uncertainty, a stricter-cutoff sensitivity case, assumptions, risks, and monitoring actions.
- The report is regenerated by `spy_stakeholder_delivery.ipynb` and the Stage12 cell in `project_pipeline.ipynb`; the underlying model is not refit during delivery.


## Productization and Local API

- Stage13 saves the fitted chronological baseline plus feature schema and decision metadata to `model/spy_risk_alert_model.pkl`. The model is loaded once when `app.py` starts; it is never re-fit by an API request.
- From `project/`, run `python app.py`, then call `GET http://127.0.0.1:5050/health`. Send `POST /predict` with `{"features": {"<each model feature>": <finite number>}}`; feature keys must exactly match the saved schema. The JSON response contains `probability`, `alert`, `cutoff`, and the training event threshold. Invalid inputs return JSON HTTP 400.
- Run `notebooks/spy_productization.ipynb` to reproduce API requests, including a deliberately invalid payload. See [stakeholder handoff](docs/stakeholder_handoff.md) for the decision boundary, risks, and operational instructions.

### Reproduce from a fresh pull
...
The full integration flow remains `notebooks/project_pipeline.ipynb`. To deterministically rebuild only the stakeholder report from persisted Stage10/11 artifacts, run from `project/`:

```bash
python -m src.run_step --stage stakeholder-report
```
